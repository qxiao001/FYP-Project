...
public class JoinChillerData2 {
	...
	public void executeJoin() throws Exception {
		...
		insertResult(rs2, "chiller2pt");
		 ...
		 joinDB("final");
		 callR();
		 joinDB("updateCombfinalts");

	}

	private void getMax5432() throws SQLException {
		Statement st = null;
		ResultSet rs = null;
	...
	private void insertResult(ResultSet rs, String table)
			throws SQLException {
	...
	private void callR() throws Exception {
		// TODO Auto-generated method stub
		
		String content=readFromJARFile("regressionDB.R");
		...
		System.out.println("Done of generating R scrcipt"+content);
		...
		try {
			...
			shell = Runtime.getRuntime().exec("R --vanilla --file=regressionDB2.R");
	...
	public Connection getDBConn(String conn, String user, String pwd) {
		try {
	...
	public ResultSet joinDB(String tableName) throws Exception {
		Statement st = null;
		ResultSet rs = null;
		...
		case "updateMax":
			...
			break;
		case "updateCombfinalts":
			queryPath = Config.updateCombfinaltsSQLPath;
			break;
		case "indivSelect":
		...
		query = readFromJARFile(queryPath);
